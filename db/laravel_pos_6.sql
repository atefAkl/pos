...
CREATE TABLE IF NOT EXISTS `offers` (
  `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `code` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'كود العرض الفريد',
  `name` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'اسم العرض',
  `description` text COLLATE utf8mb4_unicode_ci COMMENT 'وصف تفصيلي للعرض',
  `type` enum('percentage','fixed','buy_x_get_y','bundle') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'percentage' COMMENT 'نوع العرض: نسبة مئوية، قيمة ثابتة، اشتر X واحصل على Y، حزمة',
  `value` decimal(10,2) DEFAULT NULL COMMENT 'قيمة العرض (نسبة مئوية أو قيمة ثابتة)',
  `min_qty` int UNSIGNED DEFAULT NULL COMMENT 'الحد الأدنى للكمية المطلوبة للاستفادة من العرض',
  `max_qty` int UNSIGNED DEFAULT NULL COMMENT 'الحد الأقصى للكمية المسموح بها للاستفادة من العرض',
  `min_amount` decimal(10,2) DEFAULT NULL COMMENT 'الحد الأدنى لقيمة الطلب للاستفادة من العرض',
  `max_discount` decimal(10,2) DEFAULT NULL COMMENT 'الحد الأقصى لقيمة الخصم',
  `start_date` datetime NOT NULL COMMENT 'تاريخ بداية العرض',
  `end_date` datetime DEFAULT NULL COMMENT 'تاريخ نهاية العرض (NULL = غير محدد)',
  ...
  `usage_limit` int UNSIGNED DEFAULT NULL COMMENT 'الحد الأقصى لعدد مرات استخدام العرض',
  ...
  `applies_to` enum('all','products','categories','customers','customer_groups') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'all' COMMENT 'نطاق تطبيق العرض',
  `created_by` bigint UNSIGNED DEFAULT NULL COMMENT 'معرف المستخدم الذي أنشأ العرض',
  `updated_by` bigint UNSIGNED DEFAULT NULL COMMENT 'معرف المستخدم الذي قام بآخر تحديث للعرض',
  ...
  `deleted_at` timestamp NULL DEFAULT NULL COMMENT 'تاريخ الحذف (للحذف الناعم)',
  `product_id` bigint UNSIGNED DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `offers_code_unique` (`code`),
  KEY `offers_type_index` (`type`),
  KEY `offers_start_date_end_date_index` (`start_date`,`end_date`),
  KEY `offers_is_active_index` (`is_active`),
  KEY `offers_applies_to_index` (`applies_to`),
...
CREATE TABLE IF NOT EXISTS `offer_category` (
  ...
  `discount_value` decimal(10,2) DEFAULT NULL COMMENT 'قيمة الخصم المخصصة لهذه الفئة (إذا كانت مختلفة عن قيمة العرض الأساسية)',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `offer_category_offer_id_category_id_unique` (`offer_id`,`category_id`),
...
CREATE TABLE IF NOT EXISTS `offer_customer` (
  ...
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `offer_customer_offer_id_customer_id_unique` (`offer_id`,`customer_id`),
  KEY `offer_customer_customer_id_index` (`customer_id`)
...
CREATE TABLE IF NOT EXISTS `offer_product` (
  ...
  `discount_value` decimal(10,2) DEFAULT NULL COMMENT 'قيمة الخصم المخصصة لهذا المنتج (إذا كانت مختلفة عن قيمة العرض الأساسية)',
  `min_qty` int UNSIGNED DEFAULT NULL COMMENT 'الحد الأدنى للكمية المطلوبة لهذا المنتج',
  `max_qty` int UNSIGNED DEFAULT NULL COMMENT 'الحد الأقصى للكمية المسموح بها لهذا المنتج',
  `is_free_item` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'هل هذا المنتج مجاني كجزء من العرض (للعروض من نوع buy_x_get_y)',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `offer_product_offer_id_product_id_unique` (`offer_id`,`product_id`),
...
--
-- Table structure for table `offer_usage_log`
--

DROP TABLE IF EXISTS `offer_usage_log`;
CREATE TABLE IF NOT EXISTS `offer_usage_log` (
  `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `offer_id` bigint UNSIGNED NOT NULL COMMENT 'معرف العرض',
  `order_id` bigint UNSIGNED NOT NULL COMMENT 'معرف الطلب',
  `customer_id` bigint UNSIGNED DEFAULT NULL COMMENT 'معرف العميل',
  `discount_amount` decimal(10,2) NOT NULL COMMENT 'قيمة الخصم المطبق',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `offer_usage_log_offer_id_index` (`offer_id`),
  KEY `offer_usage_log_order_id_index` (`order_id`),
  KEY `offer_usage_log_customer_id_index` (`customer_id`)
...
('v0DbVQ6GzxUo1CRay9bN23MfNdckFhDBIar7g9S6', 1, '::1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:138.0) Gecko/20100101 Firefox/138.0', '', 1748175251),
('ZLMUY2fBbZCRrexL4YBercYUvSIqjhxOtE4Yo1zR', 1, '::1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:138.0) Gecko/20100101 Firefox/138.0', '', 1748265857);
...
--
-- Constraints for table `offer_customer`
--
ALTER TABLE `offer_customer`
  ADD CONSTRAINT `offer_customer_customer_id_foreign` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `offer_customer_offer_id_foreign` FOREIGN KEY (`offer_id`) REFERENCES `offers` (`id`) ON DELETE CASCADE;
...
--
-- Constraints for table `offer_usage_log`
--
ALTER TABLE `offer_usage_log`
  ADD CONSTRAINT `offer_usage_log_offer_id_foreign` FOREIGN KEY (`offer_id`) REFERENCES `offers` (`id`) ON DELETE CASCADE;
